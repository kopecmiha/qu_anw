...
from tools import MultipleChoiceArgs, clean_text, preprocess, get_candidate_sents, preprocess_mcq, set_seed, add_br, \
    clean_answer, similar_answer, join_answers, word_set, word_set_score
import logging
...
def generate_multiple_choice(args):
    ...
    for sent_index, _ in enumerate(filter_quotes_and_questions):
        ...
        aqs = generate('Context: ' + context + ' Answer: ', args.temperature_answer, 50, args.generate_count,
                       args.generate_size, 0.95, tokenizer, model)
        ...
        for aq in aqs:
            try:
                ...
                if len(q) > 25 and q.index('?') != -1:
                    ...
                    qqa = generate('Context: ' + context + ' Question: ' + q + ' Answer: ', args.temperature_question,
                                   30, args.generate_size, args.generate_count, 0.95, tokenizer, model_qa)

                    if args.wrong_context_size > 0:
                        rc = random.randint(0, args.context_size - args.wrong_context_size)
                        context_wrong = 'Context: ' + ' '.join(
                            [x.text for x in context_sents if x.text.find(a_orig) == -1][
                            rc:rc + args.wrong_context_size])
                    else:
                        context_wrong = ''
                    qqaw = generate(context_wrong + 'Question: ' + q + ' Answer: ', args.temperature_wrong_answer, 30,
                                    args.generate_size, args.generate_count, 0.95, tokenizer, model_qa)
                    ...
                    if word_set_score(word_set(aa), word_set(q)) < 0.5 and q.find(aa) == -1 and len(aa) > 2 and len(
                            aa) < 120:
                        if c_good[aa] / len(list_good) > 0.35:
                            if len(wrong) >= 2:
                                a_context_intersect_score = 40 * min(0.025,
                                                                     word_set_score(word_set(aa), word_set(context)))
                                if a_context_intersect_score > 0:
                                    rating = c_good[aa] / len(list_good) + a_context_intersect_score
                                    questions.append({'context': context, 'question': q, 'answer': aa, 'wrong': wrong,
                                                      'rating': rating})
                                    n_include += 1
                                    found = True
                                    break
