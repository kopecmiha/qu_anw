...
import torch
from transformers import AutoTokenizer, AutoModel, AutoModelWithLMHead, AutoModelForCausalLM, AutoModelForSeq2SeqLM, \
    ...
    for sent_index, _ in enumerate(filter_quotes_and_questions):
        try:
            ...
            for n_try in range(0, 2):
                ...
                false = [x for x in false if
                         len(x) - len(sent_part) >= true_len and len(x) - len(sent_part) <= true_len * 2][-5:]
